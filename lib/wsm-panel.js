...
/** @lends WsmPanel */
{
  ...
  attach: function() {
    ...
    Rdp.registerTabActor(client, config).then(({registrar, front}) => {
      ...
      this.front.on("onWebSocketCreated", this.onWebSocketCreated);
      this.front.on("onWebSocketOpened", this.onWebSocketOpened);
      this.front.on("onWebSocketClosed", this.onWebSocketClosed);
  ...
  detach: function() {
    ...
    front.detach().then(response => {
      front.off("onWebSocketCreated", this.onWebSocketCreated);
      front.off("onWebSocketOpened", this.onWebSocketOpened);
      front.off("onWebSocketClosed", this.onWebSocketClosed);
  ...
  onFrameReceived: function(data) {
    FBTrace.sysout("onFrameReceived", data);

    let payload = data.payload;
    this.decodePacket(data, payload);
    this.postContentMessage("frameReceived", JSON.stringify(data));
  },

  onFrameSent: function(data) {
    FBTrace.sysout("onFrameSent", data);
