...
var mqttParser = require("./mqtt-parser.js").parser;
var Buffer = require("./mqtt-parser.js").Buffer;
...
/** @lends WsmPanel */
{
  ...
  decodePacket: function(data, payload) {
    ...
    // Async, so we mutate data directly
    this.decodeMqttPacket(data, payload);
  },
   ...
   */
  decodeMqttPacket: function(frame, data) {
    try {
      ...
      // Parser.parse returns the length left to read
      while(parser.parse(buffer) > 0) {
        ;
      }
    } catch (err) {
    }
